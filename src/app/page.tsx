  ...
  return (
    <div className="flex flex-col">
      ...
      <section className="relative h-[600px] flex items-center justify-center overflow-hidden bg-slate-900">
        <div className="absolute inset-0 opacity-20 bg-[url('/Feyalegria.png')] bg-no-repeat bg-center bg-contain"></div>
        <div className="absolute inset-0 bg-gradient-to-b from-transparent to-black/60"></div>

        <div className="relative z-10 text-center px-4">
          <h2 className="text-[#dc2626] text-5xl md:text-8xl font-black italic italic tracking-tighter mb-2 drop-shadow-lg">
            Promoción 2026
          </h2>
          ...
          <div className="flex flex-wrap justify-center gap-4">
            <Link href="/noticias" className="px-10 py-4 bg-[#dc2626] text-white font-black rounded-sm hover:translate-y-[-2px] transition shadow-xl">
              VER NOTICIAS
            </Link>
            <Link href="/contacto" className="px-10 py-4 bg-white text-[#dc2626] font-black rounded-sm hover:translate-y-[-2px] transition shadow-xl">
              CONTACTAR
            </Link>
          </div>
        </div>
      ...
      <section className="py-20 bg-white">
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { title: 'PROYECTO TRANSVERSAL', color: 'bg-[#dc2626]' },
              { title: 'COMUNIDAD EDUCATIVA', color: 'bg-[#991b1b]' },
              { title: 'PLAN DE ESTUDIO', color: 'bg-[#7f1d1d]' },
              { title: 'PASTORAL', color: 'bg-slate-800' },
            ].map((item) => (
      ...
      <section className="py-20 bg-slate-50">
        <div className="container mx-auto px-4">
          <div className="flex flex-col items-center mb-12">
            ...
            <div className="h-1.5 w-24 bg-[#dc2626] mt-2"></div>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {posts.length > 0 ? posts.map((post) => (
              <article key={post.id} className="bg-white group cursor-pointer border border-slate-100 shadow-sm hover:shadow-2xl transition duration-500 rounded-none overflow-hidden">
                ...
                <div className="p-8">
                  <span className="text-[#dc2626] text-[10px] font-black tracking-[0.2em] uppercase">{post.category.name}</span>
                  ...
                  <Link href={`/noticias/${post.id}`} className="text-xs font-black text-slate-900 border-b-2 border-[#dc2626] pb-1 hover:text-[#dc2626] transition">LEER MÁS</Link>
                </div>
              </article>
            )) : (
      ...
      {/* Events Banner */}
      <section className="bg-[#dc2626] py-16 text-white">
        <div className="container mx-auto px-4 flex flex-col md:flex-row items-center justify-between gap-8">
          ...
          <Link href="/eventos" className="px-10 py-4 bg-white text-[#dc2626] font-black hover:bg-slate-100 transition whitespace-nowrap shadow-xl">
            VER CALENDARIO
          </Link>
        </div>
      </section>
